...
router.get(`/:id`, async (req, res, next) => {
  if (req.session.user) {
        ...
        await dbClient
          ...
          .then((results) => {
            ...
            if (req.session.user.category == 3) {
              ...
              dbClient.query(find_center_id_query).then((ans) => {
            ...
            } else if (req.session.user.category == 2) {
              ...
              dbClient.query(find_parent_id_query).then((ans) => {
                res.render(`ticketing`, {
                  count_current: results.rows[0].count,
                  count_max: results.rows[0].count_max,
                  post_id: req.params.id,
    ...
    res.render("alert", {
      error: "로그인 정보를 찾을 수 없습니다. 다시 로그인해주세요.",
    });
  }
});
